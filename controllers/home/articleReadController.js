...
module.exports.like_article = async (req,res) => {
    ...
    try {
        ...
        if(!like_status && !dislike_status){
            ...
            res.status(200).json({ successMessage: 'you liked this article' })
        }
            ...
            await notificationModel.create({
                subject: `${userName} undo like your article`,
                slug,
                adminId
            });
            res.status(200).json({ successMessage: 'you undo like this article' })
        }
        else if(!like_status && dislike_status){
            await articleModel.updateOne(
                { 
...
module.exports.dislike_article = async (req,res) => {
    ...
    try {
        ...
        if(!like_status && !dislike_status){
            ...
            res.status(200).json({ successMessage: 'you disliked this article' })
        }
        else if(!like_status && dislike_status){
            await articleModel.updateOne(
                { _id: articleId },
            ...
            await notificationModel.create({
                subject: `${userName} undo dislike your article`,
                slug,
                adminId
            });
            res.status(200).json({ successMessage: 'you undo dislike this article' })
        }
        else if(like_status && !dislike_status){
            await articleModel.updateOne(
                { 
